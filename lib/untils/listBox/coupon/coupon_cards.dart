...
class CouponCards extends StatefulWidget {
  @override
  _CouponCardsState createState() => _CouponCardsState();
}

class _CouponCardsState extends State<CouponCards> {
  ...
  Widget adds(){
    ...
    return Container(
        ...
        child:GestureDetector(
          ...
          onTap: ()async{
            await _vitalModels.addCoupon(sex);
  ...
  List<Widget> ConmodiyShops (){
    ...
    for(var i = 0; i <_vitalModels.PrototypeDatas["list"][0]["items"].length; i++){
      if(_vitalModels.PrototypeDatas["list"][0]["items"][i]['id'] == 'coupon'){
        lengths.add(i);
      }
    }
    List<Widget> _A = [];
    for (var i =0; i<lengths.length; i++){
      ...
      _A.add(
          ExpansionTile(
            title: Text("优惠券组${i+1}", style: TextStyle(fontSize: ScreenUtil().setSp(28)),),
          ...
          Expanded(
              child: GestureDetector(
                onTap: () async {
                  ...
                  await _vitalModels.setBackground(result);
                },
                child: Container(
