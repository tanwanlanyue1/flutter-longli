...
import 'picturew_item.dart';

class PicturewCards extends StatefulWidget {
  @override
  _PicturewCardsState createState() => _PicturewCardsState();
}

class _PicturewCardsState extends State<PicturewCards> {
  @override
  ...
  Widget adds(){
    ...
    return Container(
        ...
        child:GestureDetector(
          ...
          onTap: ()async{
            await _vitalModels.addPicturew();
  ...
  List<Widget> ConmodiyShops (){
    ...
    for(var i = 0; i <_vitalModels.PrototypeDatas["list"][0]["items"].length; i++){
      if(_vitalModels.PrototypeDatas["list"][0]["items"][i]['id'] == 'picturew'){
        lengths.add(i);
      }
    }
    List<Widget> _A = [];
    for (var i =0; i<lengths.length; i++){
      ...
      _A.add(
          ExpansionTile(
            title: Text("图片橱窗组${i+1}", style: TextStyle(fontSize: ScreenUtil().setSp(28)),),
