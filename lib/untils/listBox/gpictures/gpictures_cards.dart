...
class _GpicturesCardState extends State<GpicturesCard> {
  ...
  Widget adds(){
    ...
    return Container(
        ...
        child:GestureDetector(
          ...
          onTap: ()async{
            await _vitalModels.addGpictures();
  ...
  List<Widget> ConmodiyShops (){
    ...
    for(var i = 0; i <_vitalModels.PrototypeDatas["list"][0]["items"].length; i++){
      if(_vitalModels.PrototypeDatas["list"][0]["items"][i]['id'] == 'gpictures'){
        lengths.add(i);
      }
    }
    List<Widget> _A = [];
    for (var i =0; i<lengths.length; i++){
      ...
      _A.add(
          ExpansionTile(
            title: Text("商品橱窗组${i+1}", style: TextStyle(fontSize: ScreenUtil().setSp(28)),),
