...
class _PicturesCardState extends State<PicturesCard> {
  ...
  Widget adds(){
    ...
    return Container(
        ...
        child:GestureDetector(
          ...
          onTap: ()async{
            await _vitalModels.addPictures();
  ...
  List<Widget> ConmodiyShops (){
    ...
    for(var i = 0; i <_vitalModels.PrototypeDatas["list"][0]["items"].length; i++){
      if(_vitalModels.PrototypeDatas["list"][0]["items"][i]['id'] == 'pictures'){
        lengths.add(i);
      }
    }
    List<Widget> _A = [];
    for (var i =0; i<lengths.length; i++){
      ...
      _A.add(
          ExpansionTile(
            title: Text("图片展播组${i+1}", style: TextStyle(fontSize: ScreenUtil().setSp(28)),),
