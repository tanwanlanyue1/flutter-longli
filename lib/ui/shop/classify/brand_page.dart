...
//  String _checkTag = "↑";
  ...
  _brandList ({int page}) async {
    ...
    if (result["code"] == 0) {
      setState(() {
//        if(pages==1){
          brand = result["data"]["records"];
        lengths = result["data"]["total"];
//          pages++;
//        }else{
//          brand += result["data"]["records"];
//        }
  ...
  void initState() {
    ...
    _footController.addListener(() {
      print("???????");
      if (_footController.position.pixels == _footController.position.maxScrollExtent-200) {
        //表示你滚动的位置是否达到了最大滚动的位置也就是底部，如果 是执行——loadData这个方法
        if(lengths<999){
          _brandList(page: pages);
        }else{
          print(">>>$lengths");
          _brandList(page: pages);
  ...
  void initData() {
    for(var i=0;i<brand.length;i++){
      if(brand[i]["firstChar"]=="A"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'A',imgs: brand[i]["image"]),);
      }else if(brand[i]["firstChar"]=="B"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'B',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="C"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'C',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="D"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'D',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="E"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'E',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="F"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'F',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="G"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'G',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="H"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'H',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="I"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'I',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="J"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'J',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="K"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'K',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="L"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'L',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="M"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'M',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="N"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'N',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="O"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'O',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="P"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'P',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="Q"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'Q',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="R"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'R',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="S"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'S',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="T"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'T',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="U"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'U',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="V"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'V',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="W"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'W',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="X"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'X',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="Y"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'Y',imgs: brand[i]["image"]));
      }else if(brand[i]["firstChar"]=="Z"){
        contactList
            .add(ContactInfo(name: '${brand[i]["cnName"]}', namePinyin: 'Z',imgs: brand[i]["image"]));
      }
    }
  }
  @override
...
// 展示的内容
  _buildListItem(ContactInfo model,) {
    ...
    Map brandNameList = {'brandNameList':null};
    return Column(
      children: <Widget>[
        Offstage(
                        ...
                        child: Row(
                          children: <Widget>[
                            Container(
                              ...
                              height: 40,
                              child: Image.network(ApiImg+model.imgs,fit: BoxFit.fill,),
                            ),
                            Padding(
                              padding:  EdgeInsets.fromLTRB(15.0, 0.0, 0.0, 0.0),
                              child:  Text(model.name,style: TextStyle(fontSize: ScreenUtil().setSp(30),color: Color(0xff8A8A8A)),),
                            ),
                          ],
                        ),
                      ),
                      onTap: (){
                        brandNameList["brandNameList"] = model.name;
                        Navigator.pushNamed(context, '/secondClassifyPage',arguments:brandNameList);
                      },
                    ),
                    Container(
...
class ContactInfo extends ISuspensionBean {
  String name;
  String namePinyin;
  String imgs = "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1565845607483&di=ea898b76a87bca218b649c3afff76f3b&imgtype=0&src=http%3A%2F%2Fimg3.duitang.com%2Fuploads%2Fitem%2F201601%2F24%2F20160124110308_Vy8cw.jpeg";
